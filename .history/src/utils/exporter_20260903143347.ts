...
/** Genera un documento Word compatible y dispara la descarga. */
export function exportDictamenWord(data: DictamenData) {
  ...
  const chapterRows = CHAPTERS.map((chapter) => {
    const deficiency = data.deficiencias[chapter.id];
    return [
      chapter.code,
      chapter.name,
      deficiency?.clase ?? "—",
      deficiency ? `${deficiency.percent}%` : "Sin calificar",
      `${chapter.maxPercent}%`,
    ];
  });
  ...
  const category = data.pcl.total >= 50 ? "SEVERA" : data.pcl.total >= 25 ? "MODERADA" : "LEVE";
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Dictamen MedValua</title>
    <style>body{font-family:Arial,sans-serif;color:#202020;margin:40px}h1{color:#168a7a;font-size:24px;margin-bottom:4px}h2{color:#168a7a;font-size:16px;border-bottom:1px solid #168a7a;padding-bottom:5px;margin-top:24px}p{font-size:11pt}table{border-collapse:collapse;width:100%;margin:8px 0 18px}th{background:#168a7a;color:white;text-align:left}th,td{border:1px solid #c7d9d5;padding:6px;font-size:10pt}td:first-child{font-weight:bold}.pcl{background:#168a7a;color:white;padding:14px;margin:18px 0}.pcl strong{font-size:25px}</style></head>
    <body><h1>MedValua</h1><p>Dictamen Pericial · Decreto 1507/2014 — Colombia</p>
    <p><strong>Emitido:</strong> ${escapeHtml(new Date().toLocaleString("es-CO"))}</p>
    <h2>1. Datos del Paciente</h2>${table(["Campo", "Valor"], patientRows)}
    <div class="pcl"><strong>PÉRDIDA DE CAPACIDAD LABORAL (PCL): ${escapeHtml(data.pcl.total)}%</strong><br>
      Título I: ${escapeHtml(data.pcl.tituloI)} / 50 · AVD: ${escapeHtml(data.pcl.avd)} / 25 · Rol Laboral: ${escapeHtml(data.pcl.rolLaboral)} / 25<br>
      Categoría: ${category}</div>
    <h2>2. Título I — Deficiencias (combinado: ${escapeHtml(data.tituloIPercent)}%)</h2>
    ${table(["Cap.", "Sistema", "Clase", "Asignado", "Tope"], chapterRows)}
    <h2>3. Título II — Roles</h2>
    ${table(["Componente", "Puntos", "Máximo"], [["Actividades de la Vida Diaria (AVD)", String(data.tituloII.avd), "25"], ["Rol Laboral", String(data.tituloII.rolLaboral), "25"]])}
    <p><em>Fórmula PCL = Título I (50%) + AVD (25%) + Rol Laboral (25%). Combinación intracapítulo por fórmula de Balthazard: A + (100−A)×(B/100).</em></p>
    </body></html>`;
  downloadBlob(
    `medvalua-${slugify(data.paciente.nombre)}-${new Date().toISOString().slice(0, 10)}.doc`,
    html,
